...
class LoadPlanTest(unittest.TestCase):
    def test_reads_rows_into_tasks(self):
        path = write_csv(
            'M-1,Лид,Веха 1,lead,2026-09-19,2026-09-19,веха,,critical,"Всё лежит в репо"\n'
            'ML-02,ML,JSON,ml,2026-09-17,2026-09-18,,M-1,critical,"Поля добавлены"\n'
        )
        tasks = load_plan(path)
        self.assertEqual(len(tasks), 2)
        self.assertEqual(tasks[0], PlanTask(
            key="M-1", component="Лид", title="Веха 1", assignee="lead",
    ...
    def test_validate_reports_broken_link_and_bad_dates(self):
        path = write_csv(
            'A-1,ML,Задача,ml,2026-09-20,2026-09-18,,M-9,normal,"x"\n'
        )
    ...
    def test_validate_reports_duplicate_keys(self):
        path = write_csv(
            'A-1,ML,Задача,ml,2026-09-17,2026-09-18,,,normal,"x"\n'
            'A-1,ML,Задача 2,ml,2026-09-17,2026-09-18,,,normal,"y"\n'
        )
...
class HelpersTest(unittest.TestCase):
    ...
    def test_description_contains_criterion_and_key(self):
        task = PlanTask("FE-01", "Фронт", "Каркас", "product", "2026-09-17",
    ...
    def test_description_starts_with_goal_when_present(self):
        task = PlanTask("FE-01", "Фронт", "Каркас", "product", "2026-09-18",
