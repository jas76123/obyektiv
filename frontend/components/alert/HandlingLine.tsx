...
export function HandlingLine(p: {
  ...
  const closeBtn = <button className="act" type="button" aria-expanded={p.open === 'outcome'} onClick={p.onToggleOutcome}>{p.open === 'outcome' ? 'Отменить' : 'Зафиксировать исход'}</button>;
  ...
  const contactBtn = <button className="act primary" type="button" aria-expanded={p.open === 'contact'} onClick={p.onToggleContact}>{p.open === 'contact' ? 'Свернуть контакт' : `Связаться · ${short}`}</button>;
  if (h.state === 'new') {
