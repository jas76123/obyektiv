...
describe('postShot', () => {
  ...
  it('sends correct multipart with all required fields', async () => {
    const mock = vi.fn(async (_url, init) => {
      ...
      expect((photo as File).name).toBe('br-1.t1.a.jpg');
      expect(fd.has('retake_of')).toBe(false);
      return new Response(JSON.stringify({ server_id: 'srv-a', status: 'uploaded' }), { status: 201 });
    });
    vi.stubGlobal('fetch', mock);
    const r = await postShot('http://x', rec(), new Blob(['x'], { type: 'image/jpeg' }), 'br-1');
    expect(r).toEqual({ server_id: 'srv-a' });
  });

  it('без бригады имя файла начинается с unknown', async () => {
    const mock = vi.fn(async (_url, init) => {
      expect(((init.body as FormData).get('photo') as File).name).toBe('unknown.t1.a.jpg');
      return new Response(JSON.stringify({ server_id: 'srv-c', status: 'uploaded' }), { status: 201 });
    });
    vi.stubGlobal('fetch', mock);
    await postShot('http://x', rec(), new Blob(['x'], { type: 'image/jpeg' }));
